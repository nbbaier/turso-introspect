...
async function getSchemaSql(source: string, options: DiffOptions, logger: Logger): Promise<string> {
  if (existsSync(source)) {
    try {
      const stats = await fs.stat(source);
      if (stats.isDirectory()) {
        throw invalidArgsError(`"${source}" is a directory, expected a file or database`);
      }
      if (stats.isFile()) {
...
export async function diff(db1: string, db2: string, options: DiffOptions) {
  ...
  if (diffFormat !== 'diff' && diffFormat !== 'migration') {
    throw invalidArgsError(`Invalid --diff-format: "${diffFormat}". Use "diff" or "migration".`);
  }
  ...
  if (diffFormat === 'migration') {
    logger.warn('"migration" format is not fully implemented yet. Falling back to unified diff.');
  }
