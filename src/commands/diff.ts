...
export function stripGeneratedHeader(sql: string): string {
	...
	// Only strip when the input actually carries the generated banner:
	// the first non-blank line must be the turso-introspect marker.
	// Hand-written files with leading "-- Database:" comments are untouched.
	const firstNonBlank = lines.find((line) => line.trim() !== "");
	if (
		!firstNonBlank?.trim().startsWith("-- Schema generated by turso-introspect")
	) {
		return sql;
	}
